...
`enterText` taps the element to focus it, then sends one key event per
character. `SlintView` turns `KeyEvent.character` into Slint text input, so
this exercises the app's real keyboard path. Only characters, backspace, and
enter are mapped today — the same limits `SlintView` itself has.
...
[`slint_testing`](../slint_testing) tests a component **headlessly**: no
