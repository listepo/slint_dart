...
@ffi.DefaultAsset('package:todo_example/todo.g.dart')
library;
...
/// AOT-compiled `TodoApp` — drives the slint-build generated component in
/// the app's `slint-dart-aot` code asset. No interpreter involved.
class TodoApp extends _AotComponent {
  TodoApp._(ffi.Pointer<ffi.Void> handle) : super(handle, _todoAppOps);
  ...
  static Future<TodoApp> create() async {
    ...
    return TodoApp._(handle);
  }

  List<Object?> get todoModel => getProperty('todo-model') as List<Object?>;
  set todoModel(List<Object?> value) => setProperty('todo-model', value);

  void onAddTodo(SlintCallbackHandler handler) => setCallbackHandler('add-todo', handler);
  Object? invokeAddTodo([List<Object?> arguments = const []]) => invokeCallback('add-todo', arguments);

  void onRemoveDone(SlintCallbackHandler handler) => setCallbackHandler('remove-done', handler);
  Object? invokeRemoveDone([List<Object?> arguments = const []]) => invokeCallback('remove-done', arguments);

  void onToggleTodo(SlintCallbackHandler handler) => setCallbackHandler('toggle-todo', handler);
  Object? invokeToggleTodo([List<Object?> arguments = const []]) => invokeCallback('toggle-todo', arguments);
}
