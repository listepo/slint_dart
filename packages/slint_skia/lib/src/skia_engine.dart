...
typedef SlintSkiaDefinitionsNameNative = Pointer<Char> Function(
    Pointer<Void> definition, Size index);
typedef SlintSkiaDefinitionsName = Pointer<Char> Function(
    Pointer<Void> definition, int index);

typedef SlintSkiaDefinitionsFreeNative = Void Function(Pointer<Void> definition);
typedef SlintSkiaDefinitionsFree = void Function(Pointer<Void> definition);
...
final _definitionsName = skiaLib
    .lookup<NativeFunction<SlintSkiaDefinitionsNameNative>>(
        'slint_skia_definitions_name')
    .asFunction<SlintSkiaDefinitionsName>();

final _definitionsFree = skiaLib
    .lookup<NativeFunction<SlintSkiaDefinitionsFreeNative>>(
        'slint_skia_definitions_free')
    .asFunction<SlintSkiaDefinitionsFree>();
...
class SkiaSlintEngine implements SlintEngine {
  ...
  List<SlintComponentDefinition> compile(
    ...
    try {
      ...
      final namePtr = _definitionsName(defPtr, 0);
      if (namePtr == nullptr) {
        _definitionsFree(defPtr);
