    ...
    pub fn set_callback_json(
        ...
        self.0
            .set_callback(name, move |args: &[Value]| {
                // An argument the bridge cannot carry (image, brush) arrives
                // as null; the call itself must still reach the handler.
                let arr: Vec<JsonValue> = args
                    .iter()
                    .map(|v| value_to_json_internal(v).unwrap_or(JsonValue::Null))
                    .collect();
                if let Ok(json_str) = serde_json::to_string(&JsonValue::Array(arr)) {
                    f(&json_str);
                }
                Value::Void
            })
...
fn value_to_json_internal(v: &Value) -> Result<JsonValue, String> {
    match v {
        ...
        // Enums read as their value name; setting one needs the enum type,
        // which JSON does not carry.
        Value::EnumerationValue(_, v) => Ok(JsonValue::String(v.to_string())),
        Value::Model(m) => {
            let mut arr = Vec::new();
